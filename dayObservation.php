<?php
echo '<div>'.$now_dateJP.'('.$week[$now_week].')';
if(count($Observation)>0){
    echo '<select id="ObservationSelect" name="ObservationSelect" onchange="ObservationSelect_change();">';
    for($cnt=0;$cnt<count($Observation);$cnt++){
        $tmp='<option value='.$cnt;
        if($cnt==count($Observation)-1){
            $tmp.=' selected=true';
        }
        $tmp2=$cnt+1;
        $tmp.='>'.$tmp2.'回目</option>';
...
echo '</div>';
for($cnt=0;$cnt<count($Observation);$cnt++){
    echo '<form id="'.$cnt.'ObservationForm" action="observe.php" method="post" class="';
    if($cnt==count($Observation)-1){
        echo 'oneSentence';
    }else{
        echo 'none';
    ...
    $tmp=$daySum+$cnt+1;
    echo '"><div class="oneSentence"><div class="oneSentence">累計'.$tmp.'回目　';
    ...
    if($_SESSION["nowToDoView"]==7){
        echo '<div class="oneSentence"><textarea cols=200 rows=2 disabled=true>'.$ImaginationTextComplete[$Observation[$cnt][22]][4].'</textarea></div>について想像<br>';
    }
    for($cnt2=0;$cnt2<2;$cnt2++){
        ...
        for($cnt3=0;$cnt3<5;$cnt3++){
            ...
            if($cnt3==3){
                echo '</div><div class="oneSentence"><select id="'.$cnt.$ObservationName[1][$cnt3].$ObservationName[0][$cnt2].'Select" name="'.$cnt.$ObservationName[1][$cnt3].$ObservationName[0][$cnt2].'Select" disabled=true>';
                ...
                for($cnt4=0;$cnt4<count($ObservationNameJP[2+$cnt3*2]);$cnt4++){
                    ...
                    echo '<input id="'.$cnt.$ObservationName[1][$cnt3].$ObservationName[0][$cnt2].'Check'.$cnt4.'" name="'.$cnt.$ObservationName[1][$cnt3].$ObservationName[0][$cnt2].'Check'.$cnt4.'" type="checkbox" disabled=true ';
                    if(substr($Observation[$cnt][3+$cnt3+$cnt2*6],$cnt4,1)==1){
                        echo 'checked';
                    }
                    echo '><label for="'.$cnt.$ObservationName[1][$cnt3].$ObservationName[0][$cnt2].'Check'.$cnt4.'">'.$ObservationNameJP[2+$cnt3*2][$cnt4].'</label>';
        ...
        echo '<div id="'.$cnt.'OtherText'.$ObservationName[0][$cnt2].'Div" class="';
        if(substr($Observation[$cnt][7+$cnt2*6],$textDisplay[0],1)!=1){
            echo ' none';
        }
        echo'">'.$ObservationNameJP[11][0].'<textarea id="'.$cnt.'OtherText'.$cnt2.'" name="'.$cnt.'OtherText'.$cnt2.'" cols=100 rows=1 disabled=true>'.$Observation[$cnt][8+$cnt2*6].'</textarea>　　</div></div>';
    }
    ...
    echo '<select id="'.$cnt.$toDoName[$_SESSION["nowToDoView"]-2].$ObservationName[2][0].'Select" name="'.$cnt.$toDoName[$_SESSION["nowToDoView"]-2].$ObservationName[2][0].'Select" disabled=true>';
    ...
    for($cnt2=0;$cnt2<3;$cnt2++){
        ...
        echo ' textAlignLeft" id="'.$cnt.$toDoName[$_SESSION["nowToDoView"]-2].$ObservationName[2][1+$cnt2*2].'Div" ';
        
        echo '>'.$ObservationNameJP[14+$cnt2*3][0].'<select id="'.$cnt.$toDoName[$_SESSION["nowToDoView"]-2].$ObservationName[2][1+$cnt2*2].'Select" name="'.$cnt.$toDoName[$_SESSION["nowToDoView"]-2].$ObservationName[2][1+$cnt2*2].'Select" disabled=true>';
        ...
        echo '</select>　<div id="'.$cnt.$ObservationName[2][2+$cnt2*2].'Div" ';
        if(!$Observation[$cnt][16+$cnt2*2]==$textDisplay[$cnt2+1]){
            echo 'class="none"';
        }else{
            echo 'class="oneSentence"';                            
        }
        echo '>'.$ObservationNameJP[16+$cnt2*3][0].'<textarea id="'.$cnt.$ObservationName[2][2+$cnt2*2].'" name="'.$cnt.$ObservationName[2][2+$cnt2*2].'" cols=100 rows=1 ';
        echo ' disabled=true>'.$Observation[$cnt][17+$cnt2*2].'</textarea></div></div>';

    }
    if($_SESSION["nowToDoView"]==6){
        echo '<div class="oneSentence textAlignLeft">'.$ObservationNameJP[23][0].'<select id="'.$cnt.$toDoName[$_SESSION["nowToDoView"]-2].'TimeZoneSelect" name="'.$cnt.$toDoName[$_SESSION["nowToDoView"]-2].'TimeZoneSelect" disabled=true>';
    ...
    if($_SESSION["nowToDoView"]==7){
        ...
        for($cnt2=0;$cnt2<20;$cnt2++){
            ...
            echo '<textarea id="'.$cnt.$toDoName[$_SESSION["nowToDoView"]-2].'word'.$cnt2.'" name="'.$cnt.$toDoName[$_SESSION["nowToDoView"]-2].'word'.$cnt2.'" cols=60 rows=1 disabled=true>'.$Observation[$cnt][23+$cnt2].'</textarea>';
            if(($cnt2+1)%3==0||$cnt2+1==20){
                echo '</div>';
            }
        }
    }
    echo '<div class="oneSentence"><button id="'.$cnt.$toDoName[$_SESSION["nowToDoView"]-2].'Update" name="'.$cnt.$toDoName[$_SESSION["nowToDoView"]-2].'Update" value='.$cnt.'>変更</button></div>';
...
if($_SESSION["nowToDoView"]==7&&$showImaginationText==1){
    ...
    echo '<label id="plusImaginationTextOpen" for="plusImaginationTextCheck"><img src="image/plus.png" alt="追加" width="40" height="40">'.$toDoNameJP[$_SESSION["nowToDoView"]-2].'のための文章の追加</label>';
    ...
    echo '<div id="textDescription"><label for="textDescriptionCheck">閉じる</label><br>'.$textDescription.'</div><br>';
