...
if($_SESSION["dDelay"]!=0){
    for($cnt=0;$cnt<count($FunEventsAbstract);$cnt++){ 
        ...
        $tmp=$FunEventsAbstract[$cnt][1]+1;
        echo $tmp.'<textarea id="FunEventsAbstractInput'.$cnt.'" name="FunEventsAbstractInput'.$cnt.'" cols="100" rows="2" disabled=true>'.$FunEventsAbstract[$cnt][5].'</textarea>';
        echo '</div>';
    }
    ...
    if($completeFunAbSum<$toDoSum[0][1]){
        ...
        if(count($FunEventsAbstract)>$completeFunAbNum){
            $tmp=$FunEventsAbstract[$completeFunAbNum][1]+1;
            echo $tmp.'<textarea id="FunEventsAbstractInput'.$cnt.'" name="FunEventsAbstractInput'.$cnt.'" cols="100" rows="2" >'.$FunEventsAbstract[$completeFunAbNum][5].'</textarea>';
        }else{
            $tmp=$biggestFunAbNum+1;
    ...
    for($cnt=0;$cnt<count($FunEventsAbstract);$cnt++){ 
        echo '<div class="oneSentence">'.$toDoNameJP[0];
        $tmp=$FunEventsAbstract[$cnt][1]+1;
        echo $tmp.'<textarea id="FunEventsAbstractInput'.$cnt.'" name="FunEventsAbstractInput'.$cnt.'" cols="100" rows="2" ';
        if($FunEventsAbstract[$cnt][4]==1){
            echo 'disabled=true>'.$FunEventsAbstract[$cnt][5].'</textarea>';
        }else{
            echo'>'.$FunEventsAbstract[$cnt][5].'</textarea>';
            echo '<input type="button" id="1_'.$cnt.'" value="保存" onClick="FunEventsAbstractSaveCompleteClick(this);">';
            echo '<input type="button" id="2_'.$cnt.'" value="完成" onClick="FunEventsAbstractSaveCompleteClick(this);">';
        }
        echo '</div>';
    }
    if($completeFunAbNum<$toDoSum[0][1]){
        ...
        //     echo '<div class="oneSentence">'.$toDoNameJP[0];
        //     $tmp=$biggestFunAbNum-count($FunEventsAbstract)+$cnt+1;
        //     echo $tmp.'<textarea id="FunEventsAbstractInput'.$cnt.'" name="FunEventsAbstractInput'.$cnt.'" cols="100" rows="2" ></textarea>';
        //     echo '<button id="FunEventsAbstractSave'.$cnt.'" name="FunEventsAbstractSave'.$cnt.'" value=1>保存</button>';
        //     echo '<button id="FunEventsAbstractComplete'.$cnt.'" name="FunEventsAbstractComplete'.$cnt.'" value=1>完成</button>';
        //     echo '</div>';
