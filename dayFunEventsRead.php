...
if($nowDateToDo[$_SESSION["nowToDoView"]]>0){
    if(count($FunEventsRead)>0){
        $tmp=$FunEventsRead[0][2]+1;
        echo '<form id="FunEventsReadForm" action="observe.php" method="post" class="inlineBlock"';
        echo '<div class="oneSentence">'.$tmp.'番</div>';
        echo '<div class="oneSentence"><textarea id="'.$toDoName[$_SESSION["nowToDoView"]-2].'" name="'.$toDoName[$_SESSION["nowToDoView"]-2].'" cols=200 rows=10 disabled=true>'.$FunEvents[$FunEventsRead[0][2]][1].'</textarea></div>';
        ...
        echo '</select>';
        echo '<div class="oneSentence"><textarea id="FunEventsReadConcrete" name="FunEventsReadConcrete" cols=200 rows=10 disabled=true>'.$FunEvents[0][1].'</textarea></div>';
        echo '<div class="oneSentence">を読み返しての20単語書き出し　';
        $tmp=$daySum+1;
        echo '累計'.$tmp.'回目';
        '</div>';
        ...
        echo '<input type="button" value="決定" onClick="submitClick(this);">';
        echo '</form>';
    }
}
?>